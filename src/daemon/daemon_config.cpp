...
std::unique_ptr<const mp::DaemonConfig> mp::DaemonConfigBuilder::build()
{
    ...
    return std::unique_ptr<const DaemonConfig>(new DaemonConfig{
        ...
        std::move(name_generator), std::move(ssh_key_provider), std::move(cert_provider), std::move(pub_cert_provider),
        std::move(client_cert_store), shared_logger, cache_directory, data_directory, server_address,
        pub_server_address, ssh_username, connection_type, image_refresh_timer});
}
